// va chercher la fonction startMusic dans le fichier utils.js et ui.js
import { startMusic } from "./utils.js";
import { createButton } from "./ui.js";
// évènement créé lors du chargement de la page
document.addEventListener("DOMContentLoaded", () => {
  // Variables globales
  let userAnswers = []; // Récupérer les réponses de l'utilisateur
  let answerButtons = [];// récupère les boutons réponse
  let timer;
  let seconds = 0;// initialise les second à 0 
  ...
  // récupère l'id et l'associe à la variable
  const startGameBtn = document.getElementById("start-game");
  // variable qui créé un bouton question suivante 
  const nextQuestionBtn = createButton("Question suivante", () => {
    if (currentQuestionIndex === 8) {// change le texte du bouton "Question suivante" en "Valider Quiz"  à la dernière question
  ...
  // donne une classe à la div attribué dans la variable board
  board.classList.add(
    "bg-white",
    "flex-col",
    "gap-4",
  ...
  // créé un élément canvas 
  const conffetiCanvas = document.createElement("canvas");
  // donne une classe au canvas attribué dans la variable conffetiCanvas
  ...
  // associe le canvas à son parent body
  document.body.appendChild(conffetiCanvas);

  //Chronometre
  const timerBorder = document.createElement("div");
  // attribue un style  et met la valeur none sur le display
  ...
  // initialise à 0 l'index du tableau
  let currentQuestionIndex = 0;

  function startGame() {
    //récupère la valeur renseigné par le joueur
    userName = userNameInput.value;

    if (userName !== "") {// vérifie que le champs n'est pas vide
      startMusic("assets/mp3/countdown.mp3");
      userNameContainer.style.display = "none";
      // met la valeur none sur le display
      ...
      showQuestion();// lance la fonction showQuestion qui récupère et afficha les questions
      startTimer(); //lance le chrono
    } else {// sinon lance la fonction popUpFunction avec le message entre parenthese
  ...
  function showQuestion() {
    ...
    fetch("assets/data/trivia.json")
      ...
      // quand on récupère les données json on éxécute ce qu'il y a entre crochet 
      .then(data => {
        // Data
        // créé une variable dans laquelle sera stocker la valeur questions du json avec les index des différent tableaux
        const question = data.questions[currentQuestionIndex];
        questionsLength = data.questions.length; //Sert à rendre la longueur de tableau en variable global
        // variable qui créé un élément div
        const questionContainer = document.createElement("div");
        // applique un id à la div
        ...
        // créé une div / lui donne un id / lui applique des class
        const answerContainer = document.createElement("div");
        answerContainer.setAttribute("id", "answer-container");
        answerContainer.classList.add("grid", "grid-cols-2", "gap-4", "w-full");
        // on récupère les valeurs answers  et on créé une boucle qui pour chaque valeur on créé un bouton on lui attribue des class 
        // on y ajout sa valeur en html et on le lie à sont parent
        question.answers.forEach((answer, index) => {
          ...
          // attribu le texte des réponses dans les boutons
          answerBtn.innerHTML = answer;
          // ajoute les boutons des réponses dans le tableau  answerButtons
          answerButtons.push(answerBtn);

          answerBtn.addEventListener("click", () => {
            selectionedAnswer = true;
            // pour chaque btn et sont index dans le tableau answerButtons
            answerButtons.forEach((btn, idx) => {
              // si idx est diférent de index cela rend le disabled effectif et le rend inutilisable
              if (idx !== index) {
                btn.disabled = true;
              }
            });
            // on fait démarrer l'index à 1 au lieu de 0 et on dit si index est égale à la bonne répponse donnée dans le fichier json
            if (index + 1 === question.correct_answer) {
              // ajoute +1 au score
              score++;
              // ajoute et change la couleur sur du vert
        ...
        // on lie les élément enfant à leurs parent
      ...
      // récupère l'erreur si il y en a une et on l'affiche dans la console
      .catch(error => {
        console.error(error);
      });
  }
  ...
  function nextQuestion() {
    if (!selectionedAnswer) {// si aucune réponse n'est séléctionnée cela lance le popup et sont message
      popUpFunction("Sélectionnez une réponse");
      return;
    }
    // ajoute +1 à l'index des question pour afficher la suivante
  ...
  function previousQuestion() {
    // la réponse selectionné reste sélectionné
    selectionedAnswer = true;
    //index des question prend -1 donc recule
    currentQuestionIndex--;
    board.innerHTML = "";
    nextQuestionBtn.innerHTML = "Question suivante"; // à optimiser????
    if (currentQuestionIndex >= 0) {// lance la fonction showQuestion et  affiche la question et réponse  en fonction de l'index
      showQuestion();
    }
  }
  ...
  function popUpFunction(message) {
    ...
    popUpBtn.addEventListener("click", function () {// efface le popup lors du click
  ...
  function validateQuiz() {
    ...
    // affiche différents texte en fonction du score du joueur 
    if (score == 0) {
  ...
  // fonction pour arrèter le chrono
  function stopTimer() {
    clearInterval(timer);
  }
  // fonction pour une nouvelle partie réinitialise le chrono le score  remet tout à zéro 
